 ...
 */
const getTestFromFile = async (filePath: string): Promise<Test> => {
...
export const updateTestsInDb = async () => {
	...
	const _testVersionHashes = new Set<string>()
	const _testEvaluationPromptVersionHashes = new Set<string>()
		...
		if (sections.length > 2) {
			for (let i = 1; i < sections.length - 1; i = i + 2) {
				if (sections[i].type !== 'assistant' || sections[i + 1].type !== 'user') {
					...
					throw new Error(
						`All sections in between the first and last section must alternate between 'assistant' then 'user', ending with 'user': ${file} (instead found ${sectionNames})`
					)
				}
			}
		}
	...
	await db.transaction(async tx => {
		for (const test of _tests.values()) {
			...
			for (const contentVersion of contentVersions) {
				...
				let structuredObjectVersionId: number | undefined = undefined
				...
				if (splitContent.normalizedEval) {
					...
					const evalInstructionsInstance =
						(
							await tx
					...
					await tx
						...
						.where(
							and(
								eq(testToEvaluationInstructionsRels.testVersionId, testInstance.id),
								ne(testToEvaluationInstructionsRels.evaluationInstructionsVersionId, evalInstructionsInstance.id)
							)
						)
				}
				...
				await tx.delete(testToTagRels).where(
					and(
						eq(testToTagRels.testVersionId, testInstance.id),
						notInArray(
							testToTagRels.tagId,
				...
				await tx.delete(testToSystemPromptVersionRels).where(
					and(
						eq(testToSystemPromptVersionRels.testVersionId, testInstance.id),
						notInArray(
							testToSystemPromptVersionRels.systemPromptVersionId,
							sysPromptVersionInstances.map(sysPrompt => sysPrompt.id)
						)
					)
				)
			}
		 ...
		 */
		const activateOrDeactivate = (active: boolean, type: 'test_versions' | 'eval_prompts') => {
			...
			return tx
				...
				.where(
					and(
						eq(typeProps.activeField, !active),
						arrFilterFnc(typeProps.hashField, Array.from(typeProps.hashes.keys()))
					)
				)
		}
