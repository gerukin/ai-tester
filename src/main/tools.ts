 ...
 */
const getToolFromFile = async (filePath: string): Promise<{ id: string; schema: any }> => {
...
export const updateToolsInDb = async () => {
	...
	const _toolVersionHashes = new Set<string>()
	...
	await db.transaction(async tx => {
		for (const [id, schemaObj] of _tools) {
			...
			const toolInstance =
				(await tx.insert(tools).values({ code: id }).onConflictDoNothing().returning())[0] ??
