...
type CliDeps = {
	...
	runTestsWithSync: (options?: { dryRun?: boolean }) => Promise<void>
	runEvalsWithSync: (options?: { dryRun?: boolean }) => Promise<void>
	listStatsQueries: () => Promise<void>
	runStatsQueryByDescription: (description: string) => Promise<void>
...
const parseDryRunArgs = (command: string, args: string[]) => {
	let dryRun = false
	...
	for (const arg of args) {
		...
		if (isHelpFlag(arg)) return { help: true, dryRun }
...
const parseStatsArgs = (args: string[]) => {
	...
	for (let index = 0; index < args.length; index++) {
		...
		if (isHelpFlag(arg)) return { help: true, shouldList, queryName, queryProvided }
		if (arg === '--list') {
