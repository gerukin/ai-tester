...
const readMarkdownFrontMatter = (filePath: string) =>
	extractFrontMatterAndTemplate(fs.readFileSync(filePath, 'utf-8')).templateConfig

const formatSection = (title: string, values: string[], emptyMessage: string) =>
	[`${title}:`, ...(values.length > 0 ? values.map(value => `  ${value}`) : [`  ${emptyMessage}`])].join('\n')
...
export const listAvailableTags = () =>
	sortUnique(
		listAllMdFiles(envConfig.AI_TESTER_TESTS_DIR).flatMap(file => TestListConfigSchema.parse(readMarkdownFrontMatter(file)).tags ?? [])
	)

export const listAvailablePromptCodes = () =>
	sortUnique(
		listAllMdFiles(envConfig.AI_TESTER_PROMPTS_DIR).flatMap(file => {
...
export const formatAvailableValues = (options: ListValueOptions) => {
