...
export type TestRunConfigOverrides = z.infer<typeof TestRunConfigOverridesSchema>
export type EvaluationRunConfigOverrides = z.infer<typeof EvaluationRunConfigOverridesSchema>

export const testsConfig = TestsConfigSchema
	.parse(yaml.parse(fs.readFileSync(envConfig.AI_TESTER_CONFIG_PATH, 'utf-8')))

export const mergeTestsConfigOverrides = <T extends Partial<TestsConfig>>(overrides: T): TestsConfig =>
	TestsConfigSchema.parse({
		...testsConfig,
		...overrides,
	})

export const resolveAnalysisQuery = (query: AnalysisQuery, registry = getFileBackedModelRegistry()): AnalysisQuery => ({
	...query,
