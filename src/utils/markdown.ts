 ...
 * Nested arrays are also supported. In this case the final version will be a combination of all the nested arrays.
 */
...
const getLastStandaloneEvaluationMarkerIndex = (content: string) => {
 ...
 */
export const extractFrontMatterAndTemplate = (content: string) => {
 ...
 */
export const getVersionsFromReplacements = (
	template: string,
	replacements?: Replacements | ReplacementValue,
	currentKey?: string
): string[] => {
	if (typeof replacements === 'string' || typeof replacements === 'number') {
		if (!currentKey) throw new Error('Current key is required for string replacements')
 ...
 */
export const getSectionsFromMarkdownContent = (content: string) => {
	const lastEvaluationMarkerIndex = getLastStandaloneEvaluationMarkerIndex(content)
	const preEvaluationContent =
		lastEvaluationMarkerIndex === undefined ? content : content.slice(0, lastEvaluationMarkerIndex)
	const evaluationContent =
		lastEvaluationMarkerIndex === undefined
			? undefined
	...
	const matches = preEvaluationContent.match(
		new RegExp(
			`((?:^|${USER_MESSAGE_MARKER}|${ASSISTANT_MESSAGE_MARKER})(?:.|\n)+?)(?=${USER_MESSAGE_MARKER}|${ASSISTANT_MESSAGE_MARKER}|$)`,
			'g'
		)
	)
	...
	if (!filteredMatches) throw new Error('No sections found in the markdown content')

	sections.push(
		...filteredMatches.map(section => {
			const type: SectionType = section.startsWith(USER_MESSAGE_MARKER)
				? 'user'
	...
	if (sections.length === 0) throw new Error('No sections found in the markdown content')
 ...
 */
export const sectionsToNormalizedStrings = (sections: TemplateSections) => {
	const normalizedSections = sections.map(({ content, type }) => ({
		type,
		content:
			type === 'evaluation' || type === 'system'
				? content
				: type === 'assistant'
				? `${ASSISTANT_MESSAGE_MARKER}\n\n${content}`
				: `${USER_MESSAGE_MARKER}\n\n${content}`,
 ...
 */
export const getEvalInstrFromSections = (sections: TemplateSections) => {
