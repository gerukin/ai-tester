 ...
 */
const getStructuredObjectFromFile = async (filePath) => {
...
export const updateStructuredObjectsInDb = async () => {
    ...
    const _objectVersionHashes = new Set();
    ...
    await db.transaction(async (tx) => {
        for (const [id, schema] of _objects) {
            ...
            const objectInstance = (await tx.insert(structuredObjects).values({ code: id }).onConflictDoNothing().returning())[0] ??
