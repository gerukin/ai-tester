...
import { extractFrontMatterAndTemplate, getVersionsFromReplacements, TagsValidation, ReplacementsValidation, getSectionsFromMarkdownContent, assertNoUnresolvedStaticPlaceholders, } from '../utils/markdown.js';
...
const PromptValidation = z.intersection(ConfigValidation, z.object({
    template: z.string(),
}));
/**
...
export const updatePromptsInDb = async () => {
    ...
    const _promptVersionHashes = new Set();
    for (const file of listAllMdFiles(envConfig.AI_TESTER_PROMPTS_DIR)) {
        ...
        if (prompt.tags?.includes('_evaluator')) {
            if (sections.length !== 2 || sections[0].type !== 'system' || sections[1].type !== 'user') {
                ...
                throw new Error(`Prompt ${prompt.id} with tag "_evaluator" must have sections system, and user (instead found ${sectionNames})`);
            }
        }
        else {
    ...
    await db.transaction(async (tx) => {
        for (const [id, prompt] of _prompts) {
            const promptInstance = (await tx.insert(prompts).values({ code: id }).onConflictDoNothing().returning())[0] ??
            ...
            await tx.delete(promptToTagRels).where(and(eq(promptToTagRels.promptId, promptInstance.id), notInArray(promptToTagRels.tagId, tagInstances.map(tag => tag.id))));
        }
        /**
         ...
         */
        const activateOrDeactivate = (active) => {
            const arrFilterFnc = active ? inArray : notInArray;
            return tx
                .update(promptVersions)
                .set({ active })
                .where(and(eq(promptVersions.active, !active), arrFilterFnc(promptVersions.hash, Array.from(_promptVersionHashes.keys()))));
        };
