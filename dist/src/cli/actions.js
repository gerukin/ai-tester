...
export const syncSkill = async ({ replace = false, cwd = process.cwd(), sourceDir = resolveDistributableSkillSourceDir(), confirmReplace, isInteractive = isTerminalInteractive(), } = {}) => {
    ...
    if (destinationExists && !replace) {
        ...
        const shouldReplace = confirmReplace !== undefined
            ? await confirmReplace(message)
            : isInteractive
                ? await (await import('../utils/menus.js')).askYesNo(message, false)
                : undefined;
        if (shouldReplace === undefined) {
            throw new CliUsageError(`${destinationDir} already exists. Rerun with --replace to replace it without interactive confirmation.`);
        }
        if (!shouldReplace) {
    ...
    console.log(`${destinationExists ? 'Replaced' : 'Created'} ${DISTRIBUTABLE_SKILL_NAME} skill.\nSource: ${sourceDir}\nDestination: ${destinationDir}`);
};
...
const readJsonPayload = (label, json) => {
    try {
...
const readJsonFilePayload = (label, filePath) => {
    ...
    try {
        ...
        throw new CliUsageError(`Unable to read ${label} file at ${absolutePath}: ${error instanceof Error ? error.message : String(error)}`);
    }
    return readJsonPayload(label, content);
};
const readOptionalJsonPayload = ({ label, json, filePath, }) => {
    if (json !== undefined && filePath !== undefined) {
        throw new CliUsageError(`${label} accepts only one JSON source.`);
    }
    if (json !== undefined)
        return readJsonPayload(label, json);
    if (filePath !== undefined)
        return readJsonFilePayload(label, filePath);
    return {};
};
const formatDryRunConfig = (value) => JSON.stringify(JSON.parse(stableJsonStringify(value)), null, 2);
const withTemporaryDatabaseCopy = async (action) => {
...
const resolveTestRunConfig = async ({ configOverridesJson, configOverridesFile, }) => {
    const { TestRunConfigOverridesSchema, mergeTestsConfigOverrides, resolveTestsConfig, getFileBackedModelRegistry } = await import('../config/index.js');
    const registry = getFileBackedModelRegistry();
    const overrides = TestRunConfigOverridesSchema.parse(readOptionalJsonPayload({
...
const resolveEvaluationRunConfig = async ({ configOverridesJson, configOverridesFile, }) => {
    const { EvaluationRunConfigOverridesSchema, mergeTestsConfigOverrides, resolveTestsConfig, getFileBackedModelRegistry } = await import('../config/index.js');
    const registry = getFileBackedModelRegistry();
    const overrides = EvaluationRunConfigOverridesSchema.parse(readOptionalJsonPayload({
...
export const runTestsWithSync = async ({ dryRun = false, includeCounts = false, configOverridesJson, configOverridesFile, confirmSync, confirmRun = alwaysYes, } = {}) => {
    if (dryRun) {
        const { testsConfig, missingCount } = includeCounts
            ? await withTemporaryDatabaseCopy(async () => {
                const { testsConfig } = await runQuietly(() => resolveTestRunConfig({ configOverridesJson, configOverridesFile }));
                await runQuietly(validateRunConfig);
                await runQuietly(syncAll);
                const missingCount = await runQuietly(async () => (await import('../main/sessions.js')).countMissingTests({ testsConfig }));
...
export const runEvalsWithSync = async ({ dryRun = false, includeCounts = false, configOverridesJson, configOverridesFile, confirmSync, confirmRun = alwaysYes, } = {}) => {
    if (dryRun) {
        const { testsConfig, missingCount } = includeCounts
            ? await withTemporaryDatabaseCopy(async () => {
                const { testsConfig } = await runQuietly(() => resolveEvaluationRunConfig({ configOverridesJson, configOverridesFile }));
                await runQuietly(validateRunConfig);
                await runQuietly(syncAll);
                const missingCount = await runQuietly(async () => (await import('../main/evaluations.js')).countMissingEvaluations({ testsConfig }));
                return { testsConfig, missingCount };
            })
            : await runQuietly(async () => {
...
const parseAdHocAnalysisQuery = async (payload) => (await import('../config/index.js')).AdHocAnalysisQuerySchema.parse(payload);
const validateAnalysisQueryCurrencies = async (analysisQueries) => {
    const { getFileBackedCurrencyRegistry, validateCurrencyRegistryReferences } = await import('../config/currency-registry.js');
    validateCurrencyRegistryReferences(getFileBackedCurrencyRegistry(), {
        analysisQueries,
...
const resolveAdHocAnalysisQuery = async (query) => {
...
const getSyncedResolvedAdHocAnalysisQuery = async (query) => runQuietly(async () => {
    await (await import('../main/providers.js')).updateProvidersInDb({
...
export const runStatsQueryByDescription = async (description, { dryRun = false } = {}) => {
    ...
    if (dryRun) {
        ...
        const query = (await getValidatedResolvedAnalysisQueries()).find(candidate => candidate.description === description) ??
            configuredQuery;
...
export const runStatsQueryJson = async (json, { dryRun = false } = {}) => {
...
export const runStatsQueryFile = async (filePath, { dryRun = false } = {}) => runStatsQueryJson(stableJsonStringify(readJsonFilePayload('stats --query-file', filePath)), { dryRun });
