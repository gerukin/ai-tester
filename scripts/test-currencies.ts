...
writeFile(
	path.join(modelsDir, 'gpt-4o-mini.yaml'),
	`code: gpt-4o-mini
provider: openai
...
writeFile(
	path.join(currenciesDir, 'USD.yaml'),
	`code: USD
rates:
  - rateInUSD: 1
    validFrom: 2025-01-19
...
writeFile(
	path.join(currenciesDir, 'JPY.yaml'),
	`code: JPY
rates:
  - rateInUSD: 0.0064
    validFrom: 2025-01-19
...
const { clearFileBackedCurrencyRegistryCache, getFileBackedCurrencyRegistry } = await import(
	'../src/config/currency-registry.js'
)
...
const expectThrows = async (label: string, fnc: () => unknown | Promise<unknown>, pattern: RegExp) => {
	try {
		await fnc()
...
writeFile(
	path.join(currenciesDir, 'usd-duplicate.yaml'),
	`code: usd
rates:
  - rateInUSD: 1
...
writeFile(
	path.join(currenciesDir, 'invalid-code.yaml'),
	`code: A1!
rates:
  - rateInUSD: 1
    validFrom: 2025-01-20
...
clearFileBackedCurrencyRegistryCache()
await expectThrows(
	'non-letter currency codes are rejected',
...
writeFile(
	path.join(currenciesDir, 'AUD.yaml'),
	`code: AUD
rates:
  - rateInUSD: 0.64
    validFrom: 2025-01-19
...
clearFileBackedCurrencyRegistryCache()
await expectThrows(
	'missing model cost currency is rejected',
	() => updateCurrenciesInDb(),
	/Currency registry is missing YAML files.*USD/
)
...
await expectThrows(
	'missing analysis query currency is rejected',
	() =>
		Promise.resolve(
			execFileSync(
				'node',
...
execFileSync(
	...
	],
	{
		...
		env: Object.fromEntries(
			Object.entries(process.env).filter(([key]) => key !== 'AI_TESTER_CURRENCIES_DIR' && key !== 'AI_TESTER_CONFIG_PATH')
		),
		stdio: 'pipe',
	}
)
